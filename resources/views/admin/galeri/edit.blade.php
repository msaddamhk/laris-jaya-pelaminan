...
@section('content')
    ...
    <form action="{{ route('kelola-galeri.update', $galeri->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" class="form-control" id="nama" name="judul" value="{{ $galeri->judul }}" required>
        </div>
            ...
            <select class="form-select @error('kategori_galeri_id') is-invalid @enderror" id="kategori_galeri_id"
                ...
                @foreach ($kategori_galeri as $kategori)
                    <option value="{{ $kategori->id }}"
                        {{ old('kategori_galeri_id', $galeri->kategori_galeri_id) == $kategori->id ? 'selected' : '' }}>
        ...
        <div class="form-group mb-3">
            ...
            <input type="date" class="form-control" id="tanggal" name="tanggal" value="{{ $galeri->tanggal_booking }}"
                required>
        </div>
